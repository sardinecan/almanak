		...
		Array.prototype.slice.call(child.children).forEach(function(subitem) {
			switch (subitem.localName.toLowerCase()) {
				case "xforms-value":
					...
					break;
				case "xforms-label":
					ilabel = subitem.textContent;
			}
		});
		return '<option value="' + ivalue + '">' + ilabel + '</option>';
	},
...
XsltForms_select.prototype.initBody = function() {
	...
	if (this.full) {
		...
		var v = select.value;
		select.innerHTML = (select.value === "\xA0" ? '<option value="\xA0" id="">\xA0</option>' : "") + Array.prototype.slice.call(cell.children).map(function(child) {
